...
export const connect = async() => {
    ...
    try {
        if(AMQP_CONNECTION_STRING){
          ...
          pubChannel.on('error', async (msg : any) => { console.error("pubChannel Error", msg); });
          subChannel.on('error', async (msg : any) => { console.error("subChannel Error", msg); });
...
export const sendToQueue = async(ex: any, queueName: any, message: any, delay = 0, retries = 0) => {
    try {
