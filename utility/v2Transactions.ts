...
export const sendRequestToAccounts = async (
  ...
  try {
    const url = process.env.ACCOUNT_BASE_URL;
...
export const prepareDataForWebhook = async (
  ...
  try {
    const { id, bet_amount, winning_amount, game_id, user_id, txn_id, ip } =
      betObj;
      console.log(key,"bet object ----------",betObj);
    ...
    switch (key) {
      case "DEBIT":
        ...
        obj.description = `${bet_amount.toFixed(2)} debited for ${
      ...
      case "CREDIT":
        ...
        obj.description = `${winning_amount?.toFixed(2)} credited for ${
          process.env.GAME_NAME
        } game for Round ${id}`;
        obj.txn_type = 1;
        break;
